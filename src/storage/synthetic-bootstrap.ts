  ...
  for (const directory of caseDirectories) {
    ...
    for (const filename of files.filter((name) => name.toLowerCase().endsWith(".pdf"))) {
      ...
      const insert = db.transaction(() => {
        db.prepare("INSERT INTO documents (id, case_id, filename, mime_type, sha256, page_count, text_content, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").run(id, caseId, parsed.filename, parsed.mimeType, parsed.sha256, parsed.pageCount, parsed.text, timestamp);
