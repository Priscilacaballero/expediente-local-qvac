...
type CatalogFile = { version: string; products: Array<{ id: string; language: "es" | "en"; name: string; description: string; audience: string; requirements: string; searchTerms?: string }> };
...
export async function ensureBankingData(db: Database.Database, root = resolve("data/synthetic")): Promise<void> {
  ...
  const insert = db.transaction(() => {
    const product = db.prepare("INSERT OR IGNORE INTO product_catalog (id, language, name, description, audience, requirements, version, updated_at, search_terms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    const updateProduct = db.prepare("UPDATE product_catalog SET name = ?, description = ?, audience = ?, requirements = ?, version = ?, updated_at = ?, search_terms = ? WHERE id = ?");
    for (const item of catalog.products) { const searchTerms = item.searchTerms ?? `${item.name} ${item.description} ${item.audience}`; product.run(item.id, item.language, item.name, item.description, item.audience, item.requirements, catalog.version, now, searchTerms); updateProduct.run(item.name, item.description, item.audience, item.requirements, catalog.version, now, searchTerms, item.id); }
