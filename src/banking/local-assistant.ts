...
export async function answerClient(db: Database.Database, adapter: QvacAdapter, language: "es" | "en", query: string): Promise<LocalAnswer> {
  ...
  try {
    ...
    const productFactQuestion = products.length > 0 && searchable.some((term) => ["producto", "prestamo", "cuenta", "ahorr", "loan", "product"].includes(term));
    if (local.trim() && !refusalOnly && !productFactQuestion) return { answer: local.trim(), sources, provider: "QVAC local" };
