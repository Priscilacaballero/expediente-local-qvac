...
checks.push({ name: "local-text-model", ok: config.QVAC_TEXT_MODEL === "LLAMA_3_2_1B_INST_Q4_0", detail: config.QVAC_TEXT_MODEL });
checks.push({ name: "local-vision-model", ok: config.QVAC_VISION_MODEL === "VISIONPSY_NANO_460M_MULTIMODAL_Q4_K_M" && config.QVAC_VISION_MMPROJ.length > 0, detail: `${config.QVAC_VISION_MODEL} + ${config.QVAC_VISION_MMPROJ}` });
checks.push({ name: "local-ocr-model", ok: config.QVAC_OCR_MODEL === "OCR_LATIN", detail: config.QVAC_OCR_MODEL });
